<template>
  <v-container>
    <v-form>
      <v-text-field prepend-icon="mdi-folder" @keyup="submitFunc" label="title" v-model="title"></v-text-field>
      <div class="errors" v-if="titleValidate">
        <span>{{ titleValidate }}</span>
      </div>
      <v-textarea no-resize label="description" v-model="description" prepend-icon="mdi-edit"></v-textarea>
      <v-text-field
      ...
      ></v-text-field>
      <v-btn @submit.prevent="submitFunc" type="submit" color="primary">Submit</v-btn>
    </v-form>
    <div class="img">
